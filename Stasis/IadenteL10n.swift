...
enum IadenteL10n {
    ...
    private static let english: [String: String] = [
        ...
        "登录时启动 iadente": "Launch iadente at Login",
        ...
        "通知": "Notifications",
        "控制 iadente 何时向你报告充电状态。": "Choose when iadente reports charging status.",
        ...
        "创建一项任务，让 iadente 在合适的时间自动照顾电池。": "Create a task so iadente can care for your battery at the right time.",
        "删除任务": "Delete Task",
        "添加计划任务": "Add Scheduled Task",
        "Apple 快捷指令": "Apple Shortcuts",
        "在“快捷指令”中使用“打开 URL”操作，即可控制 iadente。": "Use the Open URL action in Shortcuts to control iadente.",
        ...
        "Mac 唤醒并运行 iadente 后执行": "Run after the Mac wakes and iadente is active",
        "取消": "Cancel",
        "添加任务": "Add Task",
        ...
        "选择 iadente 用于充电上限、巡航和校准的电量数据。": "Choose the battery level iadente uses for charge limits, sailing, and calibration.",
        ...
        "iadente 会调整底层电池充电控制。设备不支持某项能力时，软件会自动退回只读监测模式。你可以随时关闭“管理充电”，恢复系统默认充电状态。": "iadente adjusts low-level battery charging controls. If a capability is unavailable, it falls back to read-only monitoring. Turn off Manage Charging at any time to restore the system default.",
        "开放源代码": "Open Source",
        "许可文本和第三方声明均随应用一同提供。": "License text and third-party notices are included with the app.",
        "iadente 是基于开源 Stasis 项目修改的 GPL-3.0 软件，并使用 SMCKit 与 Defaults 开源组件。": "iadente is GPL-3.0 software based on the open-source Stasis project and uses SMCKit and Defaults.",
        ...
        "iadente 是独立项目，与 AppHouseKitchen 不存在隶属、授权或分发关系。AlDente 是其各自权利人的商标。": "iadente is an independent project and is not affiliated with, authorized by, or distributed by AppHouseKitchen. AlDente is a trademark of its respective owner.",
        ...
        "需要在系统设置中批准 iadente 后台控制服务": "Approve the iadente background control service in System Settings",
        "重新检查授权": "Check Authorization Again",
        "请前往“系统设置 → 通用 → 登录项与扩展”批准 iadente，然后返回这里重新检查。": "Open System Settings → General → Login Items & Extensions, approve iadente, then return here and check again.",
        "当前正从安装磁盘映像运行，后台控制服务无法可靠启动。请退出 iadente，将应用拖入“应用程序”文件夹后重新打开。": "iadente is running from a disk image, so its background service cannot start reliably. Quit the app, drag it to Applications, and reopen it.",
        "当前设备不支持充电管理；iadente 将继续提供只读电池监测。": "This device does not support charging control; iadente will continue in read-only monitoring mode.",
        ...
        "决定 Mac 睡眠以及 iadente 未运行时如何保持充电状态。": "Choose charging behavior while the Mac sleeps or iadente is not running.",
        "达到上限前阻止睡眠": "Prevent Sleep Until Charge Limit",
        "睡眠时暂停充电": "Pause Charging During Sleep",
        "退出 iadente 后保持暂停": "Keep Charging Paused After Quitting",
        ...
        "由 iadente 控制指示灯": "Let iadente Control the LED",
        ...
        "好": "OK",
        "设置…": "Settings…",
        "退出 iadente": "Quit iadente",
        "iadente 状态": "iadente Status",
        "剩余时间": "Time Remaining",
        "电池模式": "Battery Mode",
        "电池功率": "Battery Power",
        "适配器功率": "Adapter Power",
