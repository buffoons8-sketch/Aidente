...
struct PowerSankeyView: View {
    ...
    private func drawTube(
        ...
        context.fill(
            ...
            with: .linearGradient(
                Gradient(colors: [
                    AidenteTheme.ocean.opacity(Layout.flowOpacity),
                    AidenteTheme.jade.opacity(Layout.flowOpacity),
                    AidenteTheme.gold.opacity(Layout.flowOpacity * 0.92),
                ]),
...
struct NodeView: View {
    ...
    private var colors: [Color] {
        if icon.contains("battery") {
            return AidenteTheme.chargingColors
        }
        if icon.contains("plug") || icon.contains("bolt") {
            return AidenteTheme.automationColors
        }
        return AidenteTheme.generalColors
    }
    ...
    var body: some View {
        ZStack {
            ...
            .fill(
                (colors.first ?? AidenteTheme.ocean).opacity(0.17)
            )
            .overlay(
                UnevenRoundedRectangle(
                    topLeadingRadius: isLeftSide ? cornerRadius : 0,
                ...
                .strokeBorder(
                    (colors.first ?? AidenteTheme.ocean).opacity(0.42),
                    lineWidth: 1
                )
            )
            .frame(width: 60)
            .shadow(
                color: (colors.first ?? AidenteTheme.ocean).opacity(0.16),
                radius: 3,
                y: 2
            )
            ...
            VStack(spacing: 4) {
                Image(systemName: icon)
                    .font(.system(size: 16))
                    .foregroundStyle(colors.last ?? AidenteTheme.sky)
                if let value {
